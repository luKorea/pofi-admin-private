 ...
 * @LastEditTime: 2022-06-22 10:01:58
  ...
  <div class="user-experience">
    ...
    >
      ...
      <template #isState="{ row }">
        {{ $filters.formatSelectTitle(row.state, userExperienceAuditList) }}
      </template>
    </page-content>
    <page-modal
...
export default defineComponent({
  name: 'contentCenterUserExperience',
  components: {
    editorTable,
    hyEditor,
    hyUpload
  ...
  setup() {
    ...
    const getData = (id: any) => {
      getItemData('contentCenterItem', { id: id, type: 1 }).then((res: any) => {
        if (!res.result) {
          res.data.isPrep
    ...
    const getFeelType = () => {
      getCommonSelectList('contentCenter', { type: 1 }).then((res: any) => {
        if (!res.result) {
