 ...
 * @LastEditors: korealu
 * @LastEditTime: 2022-02-28 09:49:11
...
import { IFeedbackType } from './types'
...
const apiList: any = {
  feedbacks: '/cms/feedback/'
}
let queryInfo: any = {
  currentPage: 1,
  pageSize: 10
}
const feedbackModule: Module<IFeedbackType, IRootState> = {
  namespaced: true,
  state() {
    return {
      feedbacksCount: 0,
  ...
  mutations: {
    changeFeedbacksList(state, feedbacksList: any[]) {
      state.feedbacksList = feedbacksList
    },
    changeFeedbacksCount(state, feedbacksCount: number) {
      state.feedbacksCount = feedbacksCount
    }
  },
  getters: {
    pageListData(state) {
