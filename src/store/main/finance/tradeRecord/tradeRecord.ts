...
import { ITradeTRecordType } from './types'
...
const apiList: any = {
  tradeRecord: '/cms/trade/'
}
const queryInfo: any = {
  currentPage: 1,
  pageSize: 10
}
const tradeRecordModule: Module<ITradeTRecordType, IRootState> = {
  namespaced: true,
  state() {
    return {
      tradeRecordCount: 0,
  ...
  mutations: {
    changeTradeRecordList(state, tradeRecordList: any[]) {
      state.tradeRecordList = tradeRecordList
    },
    changeTradeRecordCount(state, tradeRecordCount: number) {
      state.tradeRecordCount = tradeRecordCount
    }
  },
  getters: {
    pageListData(state) {
